...
static int csv_cache_lookup(const char* filename, TuningData* result) {
    for (uint64_t i = 0; i < csv_cache_count; i++) {
        if (strcmp(csv_cache[i].path, filename) == 0) {
            ...
            memcpy(result->data, csv_cache[i].data, result->size * sizeof(double));
            return 1;
        }
    }
...
static void csv_cache_store(const char* filename, const double* data, uint64_t size) {
    if (csv_cache_count >= CSV_CACHE_SIZE || !data || size == 0) return;
...
TuningData csv_load(const char* filename, const char* column_name) {
    ...
    if (csv_cache_lookup(filename, &result)) return result;
    ...
    if (!found) { fclose(fp); return result; }
    ...
    while (fgets(line, sizeof(line), fp)) {
        minicsv_parse_line(line, cols, &cols_count, 256);
        if (target_col >= cols_count) continue;
        if (idx == capacity) {
