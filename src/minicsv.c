...
static inline void _minicsv_out_col(char *const col_start, char **const cols,
...
static inline void _minicsv_parse_line_noquote(const int c, char **const pos_p,
                                               char **const col_start_p,
                                               int *const state_p,
                                               char **const cols,
                                               size_t *const cols_count_p,
                                               const size_t cols_max) {
...
static inline void _minicsv_parse_line_quote(const int c, char **const pos_p,
                                             int *const state_p) {
    if (c == MINICSV_QUOTE) {
...
char *minicsv_parse_line(char *const buf, char **const cols,
    ...
    *cols_count_p = 0;
    while ((c = *pos) != 0) {
        switch (state) {
        case 0:
            _minicsv_parse_line_noquote(c, &pos, &col_start, &state, cols,
                                        cols_count_p, cols_max);
            break;
        case 1:
...
void minicsv_trim_cols(char **const cols, const size_t cols_count) {
    for (size_t i = 0; i < cols_count; i++) {
        char *col = cols[i];
        while (*col && isspace((int)(unsigned char)*col))
            cols[i] = ++col;
        size_t col_end = strlen(col);
        while (col_end > 0 && isspace((int)(unsigned char)col[--col_end]))
            col[col_end] = 0;
    }
}
