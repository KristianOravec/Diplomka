 ...
 *
 * Combines:
 *   - per-kernel storage (the raw results pushed so far + derived trackers), and
...
struct KernelHandle {
    ...
    /* accumulated state */
    double   best_configs[CURVE_LIMIT_MAX]; /* best-so-far KERNEL times (curve fit input) */
    uint64_t best_len;
    double   best_config;          /* best KERNEL time so far */
    double   avg_runtime;          /* running mean of KERNEL times (curve/legacy) */
    double   avg_total_runtime;    /* running mean of TOTAL times (kernel+overhead) -> cost model */
     ...
     * kernel signals STOP once budget < 1. This is Python's commit-and-countdown
     * stopping rule, which the earlier raw-budget poll did NOT implement. */
    uint64_t budget;
    int      stopped;               /* latched once budget < 1 */
};

/* ---- stateless per-call budget, identical logic to local_budget_estimation ---- */
static const double* tuner_x_axis(void) {
    static double x[CURVE_LIMIT_MAX];
    static int init = 0;
    if (!init) { for (uint64_t i = 0; i < CURVE_LIMIT_MAX; i++) x[i] = (double)i; init = 1; }
    return x;
}

static uint64_t tuner_local_budget(uint64_t current, uint64_t total, double avg_rt,
        double* best_cfg, uint64_t best_len, uint64_t fit_start, uint64_t overhead,
        double hist_a, double hist_b) {
    const double* x = tuner_x_axis();
    double a, b, c;
    curve_fit((double*)x, best_cfg, best_len, fit_start, &a, &b, &c, hist_a, hist_b);
    uint64_t best_budget = minimize_total_runtime(a, b, c, current, total, avg_rt, overhead);
    uint64_t ib = (uint64_t)(best_budget + 0.5);
    return (curve_eval((double)current + ib, a, b, c) < best_cfg[best_len - 1]) ? ib : 0;
}
...
KernelHandle* initiate_kernel(const KernelConfig* cfg, const char* debug_name,
    ...
    if (!cfg || cfg->struct_size < sizeof(KernelConfig)) {
        if (out_status) *out_status = TUNER_ERR_INVALID_ARG;
        return NULL;
    }
    KernelHandle* h = calloc(1, sizeof(*h));
    if (!h) { if (out_status) *out_status = TUNER_ERR_ALLOC; return NULL; }
    ...
    /* Resolve historical data exactly as evaluator_run does. */
    if (cfg->mode == TUNER_MODE_HISTORICAL) {
        /* k == 0: fit historical curve params a, b. */
        CurveParams cp;
        get_regression_params(cfg->hist_HW, cfg->file_name, cfg->total_kernel_runs,
                              cfg->fit_start, cfg->hist_number_of_tests, &cp);
        h->hist_a = cp.a;
        h->hist_b = cp.b;
    } else if (cfg->mode == TUNER_MODE_HYBRID) {
        /* 0 < k < 1: compute historical optimum O_hist for the backstop. */
        h->hist_optimal_steps = history_run(cfg->hist_HW, cfg->file_name,
    ...
    h->budget = (h->total_kernel_runs < CURVE_LIMIT_MAX)
              ? h->total_kernel_runs : CURVE_LIMIT_MAX;
    h->stopped = 0;

    if (out_status) *out_status = TUNER_OK;
    return h;
}
...
void set_total_runs(KernelHandle* h, uint64_t total_kernel_runs) {
    if (h) h->total_kernel_runs = total_kernel_runs;
...
void push_result(KernelHandle* h, double kernel_time_us, double total_time_us) {
    ...
    h->avg_runtime       = (h->avg_runtime * (double)i + kernel_time_us) / (double)(i + 1);
    h->avg_total_runtime = (h->avg_total_runtime * (double)i + total_time_us) / (double)(i + 1);
    h->step = i;

    /* ---- mirror the batch estimator's per-step budget logic ---- */
    if (h->budget > 0) h->budget--;
    ...
    if (i > h->fit_start + 5) {
        ...
        /* COST MODEL CHANGE: the measured total runtime already includes overhead,
         * so we pass avg_total_runtime as the per-step cost and overhead = 0
         * (minimize_total_runtime uses (avg_rt + overhead) as the step cost).
         * This makes overhead MEASURED per step rather than a fixed constant. */
        uint64_t new_budget = tuner_local_budget(
            i, h->total_kernel_runs, h->avg_total_runtime,
            h->best_configs, h->best_len, h->fit_start, /*overhead=*/0,
            h->hist_a, h->hist_b);
        ...
        if (default_tuning_steps > 0) {
            ...
            double hist_remaining = (i < default_tuning_steps)
                ? (double)(default_tuning_steps - i) : 0.0;
            new_budget = (uint64_t)(rw * (double)new_budget + (1.0 - rw) * hist_remaining);
        }
    ...
    if (is_new_best) h->best_config = kernel_time_us;
    if (h->best_len < CURVE_LIMIT_MAX) h->best_configs[h->best_len++] = h->best_config;

    if (h->budget < 1) h->stopped = 1;
}

/* ---- query (stateless per-call) ---- */

uint64_t find_number_of_steps_that_should_be_tuning(KernelHandle* h) {
    if (!h || !h->seeded) return 1;   /* not started: keep tuning */
