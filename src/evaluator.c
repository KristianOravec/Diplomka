...
/* Large sentinel used while searching for minimum total runtime. */
...
typedef struct {
    uint64_t crystal_idx;
    uint64_t estimate;
    double extra_runtime;
    double miss;
} TestResult;
...
static uint64_t recommend_tuning_length_impl(uint64_t default_tuning_steps, const double* tuning_run, uint64_t tuning_run_len, uint64_t total_kernel_runs, double regression_weight, uint64_t fit_start, uint64_t overhead, double hist_a, double hist_b, double* best_configs);
...
static inline uint64_t get_curve_limit(uint64_t total_kernel_runs, uint64_t data_size) {
    uint64_t max_steps = (total_kernel_runs < data_size) ? total_kernel_runs : data_size;
    return (max_steps < CURVE_LIMIT_MAX) ? max_steps : CURVE_LIMIT_MAX;
}

static inline double total_runtime_for_step(double cumulative_tuning_runtime, uint64_t step_idx, uint64_t overhead, double best_runtime_so_far, uint64_t total_kernel_runs) {
...
static void build_data_path(const char* hw, const char* file_name, char* path, size_t path_len) {
    char benchmark[256];
    ...
    snprintf(path, path_len, "raw-data/raw-autotuning-data/%s/%s-%s", benchmark, safe_hw, safe_file_name);
}
...
static uint64_t run_history_trial(const TuningData* data, uint64_t curve_limit, uint64_t total_kernel_runs, uint64_t overhead, uint64_t* rng_state) {
    double cumsum = 0;
    double best = 0;
    ...
    for (uint64_t i = 0; i < curve_limit; i++) {
        double sample = data->data[random_index_from_state(data->size, rng_state)];
        cumsum += sample;
        if (i == 0 || sample < best) best = sample;

        double total = total_runtime_for_step(cumsum, i, overhead, best, total_kernel_runs);
 ...
 */
static TestResult run_monte_carlo_test(const TuningData* data, uint64_t curve_limit, uint64_t total_kernel_runs, uint64_t overhead, double k, uint64_t default_tuning_steps, uint64_t fit_start, double hist_a, double hist_b, MonteCarloWorkspace* ws, uint64_t* rng_state) {
    double* tuning_run = ws->tuning_run;
    
    /* Line 157 in Python: tuning_run = all_config_runtimes.sample(n = curve_limit).values */
    for (uint64_t i = 0; i < curve_limit; i++) tuning_run[i] = data->data[random_index_from_state(data->size, rng_state)];

    /* Lines 173-181 in Python: budget_estimator.tuning_length_recommendation() call */
    uint64_t estimate = recommend_tuning_length_impl(default_tuning_steps, tuning_run, curve_limit, total_kernel_runs, k, fit_start, overhead, hist_a, hist_b, ws->best_configs);
    
    /* Lines 158-171 in Python: best_so_far, tuning_costs, running_costs, crystal_ball */
    double cumsum = 0;
    double best = 0;
    double min_runtime = LARGE_RUNTIME_SENTINEL;
    uint64_t crystal_idx = 0;
    ...
    if (has_estimate_runtime) {
        extra_runtime = min_runtime > 0 ? estimate_runtime / min_runtime - 1.0 : 0;
    }
...
static uint64_t local_budget_estimation(uint64_t current, uint64_t total, double avg_rt, double* best_cfg, uint64_t best_len, uint64_t fit_start, uint64_t overhead, double hist_a, double hist_b) {
    ...
    double a, b, c;
    curve_fit(x_cache, best_cfg, best_len, fit_start, &a, &b, &c, hist_a, hist_b);
    ...
    for (uint64_t i = 1; i < limit; i++) {
        ...
        if (i > fit_start + 5) {
            ...
            if (default_tuning_steps > 0) {
                ...
                if (rw > 1.0) rw = 1.0;
                new_budget = (uint64_t)(rw * new_budget + (1 - rw) * (default_tuning_steps - i));
            }
...
uint64_t recommend_tuning_length(uint64_t default_tuning_steps, const double* tuning_run, uint64_t tuning_run_len,  uint64_t total_kernel_runs, double regression_weight, uint64_t fit_start, uint64_t overhead, double hist_a, double hist_b) {
    ...
    if (!best_configs) return 1;

    uint64_t result = recommend_tuning_length_impl(default_tuning_steps, tuning_run, tuning_run_len, total_kernel_runs, regression_weight, fit_start, overhead, hist_a, hist_b, best_configs);
    free(best_configs);
    return result;
}
...
uint64_t history_run(const char* HW, const char* file_name, uint64_t total_kernel_runs, uint64_t overhead, uint64_t number_of_tests) {
    ...
    if (curve_limit == 0) { csv_free(&data); return DEFAULT_FIT_START; }
...
#pragma omp parallel for if(number_of_tests > 32)
    for (uint64_t test = 0; test < number_of_tests; test++) {
        ...
        thread_sum[tid] += run_history_trial(&data, curve_limit, total_kernel_runs, overhead, &rng_state);
    }
...
void get_regression_params(const char* HW, const char* file_name, uint64_t total_kernel_runs, uint64_t fit_start, uint64_t number_of_tests, CurveParams* params) {
    ...
    uint64_t curve_limit = get_curve_limit(total_kernel_runs, data.size);
    if (curve_limit == 0) { csv_free(&data); params->a = 0.5; params->b = 1.0; params->c = 0; return; }
    
    if (number_of_tests == 0) { csv_free(&data); params->a = 0.5; params->b = 1.0; params->c = 0; return; }
    ...
    for (uint64_t test = 0; test < number_of_tests; test++) {
        ...
        for (uint64_t j = 1; j < curve_limit; j++) {
            if (tuning_run[j] < running_min) running_min = tuning_run[j];
    ...
    curve_fit(x_cache, avg_curve, curve_limit, fit_start, &a, &b, &c, NO_HISTORICAL_DATA, NO_HISTORICAL_DATA);
    
    /* Lines 99-101 in Python: Return a, b */
    params->a = a; params->b = b; params->c = c;
    free(tuning_run); free(avg_curve); csv_free(&data);
}
...
void evaluator_run(const EvaluatorParams* params, EvaluatorResult* result) {
    ...
    uint64_t curve_limit = get_curve_limit(params->total_kernel_runs, data.size);
    ...
    if (params->k == 0.0) {
        CurveParams cp;
        get_regression_params(params->hist_HW, params->file_name, params->total_kernel_runs, params->fit_start, params->number_of_tests, &cp);
        hist_a = cp.a;
        hist_b = cp.b;
    }
    ...
    int max_threads = evaluator_max_threads();
    MonteCarloWorkspace* workspaces = calloc((size_t)max_threads, sizeof(MonteCarloWorkspace));
    if (!workspaces) {
        csv_free(&data);
        clear_result(result);
        return;
    ...
    if (!ws_ok) {
        for (int t = 0; t < max_threads; t++) workspace_free(&workspaces[t]);
        free(workspaces);
        csv_free(&data);
        clear_result(result);
        return;
    ...
    if (!sum_extra_th || !sum_crystal_th || !sum_estimate_th || !sum_miss_th || !sum_extra_sq_th || !sum_crystal_sq_th || !sum_estimate_sq_th) {
        free(sum_extra_th);
        free(sum_crystal_th);
...
#pragma omp parallel for if(params->number_of_tests > 32)
    for (uint64_t test = 0; test < params->number_of_tests; test++) {
        ...
        TestResult res = run_monte_carlo_test(
            &data,
            curve_limit,
            params->total_kernel_runs,
            params->overhead,
            params->k,
            default_tuning_steps,
            params->fit_start,
            hist_a,
            hist_b,
            &workspaces[tid],
