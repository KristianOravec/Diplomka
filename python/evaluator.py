...
def get_full_path(HW, file_name):
    benchmark = file_name.replace("_output.csv", "")
    return DATA_PATH + benchmark + "/" + HW + "-" + file_name

# co som pochopil je, ze sa to snazi tato funkcia zistit stopping point na zaklade historickych dat???
def history(HW, file_name, total_kernel_runs, overhead, number_of_tests = 1000):
    full_file_path = get_full_path(HW, file_name)
    ...
    # absolutne netusim co su all_config_Runtimes 
...
#?
def gemm_history(file_name, total_kernel_runs, overhead): #nikde sa tato funkcia nevola idk
    return history("gemm-reduced",file_name,total_kernel_runs,overhead)

def fitting_function(x,a,b,c):
    return b/(x**a)+c

# snazi sa zistit parametre krivky na zaklade historickych dat
...
def evaluator(HW, file_name, total_kernel_runs, hist_HW, k = 1, overhead = 0, fit_start = 15, number_of_tests = 1000):
    ...
    if (total_kernel_runs < len(all_config_runtimes)): # moc netusim o co ide ale asi mi treba ten .csv file
    ...
    if (k > 0.0) and (k < 1.0): # 0 < k < 1 == hybrid
    ...
    if (k == 0.0): # no regression according to benchmark_suite_Variations.py
        a,b = get_history_regression_parameters(hist_HW, file_name, total_kernel_runs, fit_start)

    for _ in range(number_of_tests):
        curve_limit = min(max_tuning_steps,curve_limit_max) # velkost tuning priestoru aby si omylom nepresiel cely tuning priestor 
        tuning_run = all_config_runtimes.sample(n = curve_limit).values
        best_so_far = []
        best_so_far.append(tuning_run[0])
        ...
        crystal_ball = min(total_runtimes) # optimalne riesenie po prechodu tuning priestorom
